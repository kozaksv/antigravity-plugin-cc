...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [what Antigravity should investigate, solve, or continue]"
...
- Leave the model unset unless the user explicitly asks for one. If they ask for `spark`, map it to the Antigravity lite model `"Gemini 3.5 Flash (Low)"`.
- Leave `--resume` and `--fresh` in the forwarded request. The subagent handles that routing when it builds the `task` command.
- If the helper reports that Antigravity is missing or unauthenticated, stop and tell the user to run `/antigravity:setup`.
