...
description: Internal guidance for composing Antigravity prompts for coding, review, diagnosis, and research tasks inside the Antigravity Claude Code plugin
user-invocable: false
---

# Antigravity Prompting

Use this skill when `antigravity:antigravity-rescue` needs to ask Antigravity for help.

Antigravity routes prompts to a selectable model. Available model labels include Gemini 3.5 Flash (Low), Gemini 3.5 Flash (Medium), Gemini 3.5 Flash (High), Gemini 3.1 Pro (Low), Gemini 3.1 Pro (High), Claude Sonnet 4.6, Claude Opus 4.6, and GPT-OSS 120B.

Prompt Antigravity like an operator, not a collaborator. Keep prompts compact and block-structured with XML tags. State the task, the output contract, the follow-through defaults, and the small set of extra constraints that matter.

Core rules:
- Prefer one clear task per Antigravity run. Split unrelated asks into separate runs.
- Tell Antigravity what done looks like. Do not assume it will infer the desired end state.
...
- `<default_follow_through_policy>`: what Antigravity should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so Antigravity stays narrow and avoids unrelated refactors.
...
- Use `task --resume-last` for follow-up instructions on the same Antigravity conversation. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
...
- Ask Antigravity for brief, outcome-based progress updates only when the task is long-running or tool-heavy.
...
3. Decide whether Antigravity should keep going by default or stop for missing high-risk details.
...
Concrete end-to-end templates live in [references/antigravity-prompt-recipes.md](references/antigravity-prompt-recipes.md).
Common failure modes to avoid live in [references/antigravity-prompt-antipatterns.md](references/antigravity-prompt-antipatterns.md).
