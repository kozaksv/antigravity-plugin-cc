...
- When running that call in the foreground (not `run_in_background: true`), set `timeout: 600000` on the `Bash` call (the 600s ceiling Claude Code allows) and export `ANTIGRAVITY_COMPANION_TURN_TIMEOUT_MS=540000` in the command environment. A rescue task is a single `agy` turn; its internal turn budget must stay strictly under the external Bash `timeout` so the wrapper hits its own controlled timeout (and reaps the detached `agy`) before Bash kills it first. Do not leave the wrapper on its 900000ms default turn timeout for a foreground call — that is larger than the Bash ceiling and would lose the race.
