...
4. Додати легкий JSON-Schema валідатор БЕЗ мережевих залежностей (новий `lib/schema-validate.mjs` або inline-функція): підтримка `type`, `enum`, `required`, `additionalProperties:false`, `minLength`, `maxLength`, **`minimum`, `maximum`** (числові межі — обов'язково обчислюються), `items`, вкладені `properties`. Валідувати розпарсений output проти `review-output.schema.json`. Числові межі мають реально перевірятись: значення поза `[minimum, maximum]` (напр. `confidence: 2` при `maximum:1`, `line_start: 0` при `minimum:1`) → **fail** (fail-closed контракт вимагає їх дотримання, не декларування).
...
6. Bounded repair: при провалі валідації — щонайбільше **один** repair-turn (передати agy назад список помилок валідатора з вимогою повернути валідний JSON). Якщо повторно невалідний → явна помилка. Ліміт жорсткий (не зациклювати turn-бюджет).
   - **Repair-turn ОБОВ'ЯЗКОВО продовжує ТУ САМУ conversation (інакше тихе хибне схвалення)**: `runOneShot` без `resumeThreadId` стартує НОВУ бесіду (`antigravity.mjs:635,675` — `--conversation`/`-c`). Repair-turn, що передає agy лише список validation-помилок у свіжій бесіді, НЕ має контексту diff → легко поверне схема-валідний `approve` без жодного diff-контексту, перетворюючи fail-closed на тихе хибне схвалення. Тому repair-turn МАЄ передавати `resumeThreadId = conversationId` першого turn (той, що runOneShot повернув), щоб agy бачив оригінальний diff+завдання, а prompt repair-turn лише ДОДАЄ вимогу виправити JSON за списком помилок. Якщо conversationId першого turn недоступний (resume неможливий) → НЕ робити «сліпий» repair у новій бесіді; трактувати як явну помилку ревью (fail-closed), а не запускати без контексту.
...
- Числові межі: `confidence` поза `[minimum,maximum]` (напр. `2` при `maximum:1`) → fail; `line_start` нижче `minimum` (напр. `0` при `minimum:1`) → fail; граничні значення на межах — pass.
...
- **repair зберігає diff-контекст (проти тихого false approval)**: repair-turn викликається з `resumeThreadId` = conversationId першого turn (перевірити, що 2-й виклик fake-agy отримав `--conversation <той самий id>`/`-c`, а НЕ стартував свіжу бесіду). Окремий кейс: якщо conversationId першого turn відсутній → repair НЕ запускається у новій бесіді, результат = явна помилка ревью (fail-closed), НЕ «approve».
...
2. **Перевіряти результат `terminateProcessTree` + PID-reuse guard (проти orphan-процесів і kill-не-того-PID)**: `terminateProcessTree` уже повертає `{ attempted, delivered, method }` (`process.mjs:104`) — teardown ЗАРАЗ ігнорує це й усе одно видаляє job зі state (лишаючи процес orphaned і не-cancellable). Змінити (`session-lifecycle-hook.mjs:63`):
   - **PID-reuse verification ПЕРЕД signal**: між snapshot і kill worker міг завершитися, а ОС перевикористати PID → сигнал піде не тому процесу. Зберігати при старті job «birth token» — start-time процесу (напр. з `/proc/<pid>/stat` starttime на Linux; `ps -o lstart=`/`etimes` як переносний best-effort; на win32 — creation time через `wmic`/`Get-Process StartTime`) разом із pid у job handle. Перед `terminateProcessTree` перечитати поточний start-time за тим самим pid і звірити з birth token: не збіглося або pid уже не існує → НЕ слати сигнал (процес завершився/PID перевикористано), трактувати як «вже неживий».
   - `terminateProcessTree(pid, { graceMs: 2000 })`; перевірити повернутий `{ delivered }`.
   - **Failed kill → `cleanup-failed`, НЕ тихе видалення**: якщо signal доставлено, але процес не помер у grace-вікні (повторна перевірка живучості після graceMs), АБО `terminateProcessTree` кинув/повернув «не delivered» при живому pid → НЕ видаляти job зі state; позначити його статусом `cleanup-failed` (з причиною) через `updateState`, щоб він лишався видимим/cancellable, а не осиротів тихо. Видаляти зі state ЛИШЕ ті job, чий процес підтверджено мертвий (delivered+dead, або вже-неживий/PID-reuse).
...
- **PID-reuse**: job handle з pid P і birth token T; на момент teardown start-time за P ≠ T (мок) → `terminateProcessTree` НЕ викликається (kill-impl лічильник = 0), job трактується як вже-неживий.
- **failed kill → cleanup-failed**: `terminateProcessTree` повертає delivered але процес живий після grace (мок) → job НЕ видаляється зі state, статус `cleanup-failed`; успішний kill (delivered+dead) → job прибрано.

Verify: `npm test` зелений; grep `reset --hard` в `git.mjs` — беззастережного виклику нема (або обгорнутий recoverable-логікою); `hooks.json` SessionEnd = 60; teardown перевіряє повернутий результат `terminateProcessTree` (не ігнорує).
...
4. **Явний bootstrap-entrypoint `initializeState()` (обов'язковий — інакше міграція тихо не запуститься)**: НЕ покладатися на «перший `loadState`» як тригер міграції — Stop-hook і session-lifecycle-hook викликають `getConfig`/`listJobs`/`loadState` напряму, БЕЗ попереднього `ensureStateDir`/bootstrap; якщо міграція висить на `loadState`, після апгрейду хук побачить default `stopReviewGate:false` і старі active job зникнуть із видимості (тихе вимкнення review gate). Тому додати явний `initializeState()` (= `ensureStateDir` + `migrateLegacyState()` під локом), який КОЖЕН entrypoint викликає ПЕРШИМ рядком до будь-якого читання стану:
   - `stop-review-gate-hook.mjs` — до першого `getConfig`/`listJobs`;
   - `session-lifecycle-hook.mjs` — до snapshot/teardown-читання;
   - `antigravity-companion.mjs` — на старті кожної команди, що читає/пише стан;
   - `initializeState()` ідемпотентний (повторний виклик у вже-мігрованому стані — no-op, дешева перевірка наявності `state.json`/`.migrated`).
   **Одноразова міграція (обов'язкова)** всередині `migrateLegacyState()`: якщо новий `state.json` відсутній, а старий /tmp-шлях (`os.tmpdir()/antigravity-companion/<slug>-<hash>/state.json`) присутній:
   - **Безпекова передумова (обов'язкова, справжній TOCTOU-safe шлях — fd-first, НЕ `lstat`-before-read)**: `lstatSync` ПЕРЕД читанням НЕ є TOCTOU-safe — між перевіркою і подальшим `readFileSync` за шляхом лишається вікно, в якому атакер у world-writable /tmp підмінить файл на symlink (напр. на `~/.ssh/id_rsa` або на чужий `state.json` із системними PID). Правильний патерн: (1) відкрити дескриптор БЕЗ слідування symlink — `openSync(oldFile, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW)` (на win32, де `O_NOFOLLOW` не гарантується — окремий `lstatSync().isSymbolicLink()` як best-effort перед open); (2) `fstatSync(fd)` на ТОМУ САМОМУ fd і перевірити на fstat-результаті: `!stat.isSymbolicLink()`, uid (`stat.uid === process.getuid()` де є `getuid`), не world-writable (`(stat.mode & 0o022) === 0`); (3) читати вміст ЛИШЕ через `readSync(fd, …)` з того самого fd (не повторний open за шляхом), потім `closeSync(fd)`. Так перевірка й читання стосуються ОДНОГО inode — підміна шляху після перевірки не впливає. Батьківський каталог перевіряти аналогічно (`O_NOFOLLOW`/`lstat` на компонентах шляху) ДО open файла. Будь-яка умова не виконана або `O_NOFOLLOW` кинув `ELOOP` → **пропустити міграцію** з `logNote` (не читати, не термінувати PID), бо /tmp world-writable: атакер міг підкласти `state.json` із чужими job-PID (напр. системних сервісів), і teardown викликав би `terminateProcessTree` на них (LPE/arbitrary-kill).
   - **Уникнути рекурсивного локу (O_EXCL нереентрантний)**: `loadState` НЕ виконує міграцію через `updateState`/публічний `saveState` (вони беруть `state.lock`; якщо `loadState` викликаний уже під локом з `updateState`→read, повторний захват задедлочить). Міграція виконується ЛИШЕ в `migrateLegacyState()` (викликаний з `initializeState()` — верхньорівневий не-локнутий шлях), і пише через `saveStateUnlocked` під ОДНИМ `state.lock`, взятим саме цією точкою; наявність легасі-файлу перевіряється вже під тим локом (щоб два паралельні entrypoint не мігрували двічі). `loadState` лишається чистою читалкою без побічної міграції.
   - імпортувати щонайменше `config` **весь** (з `stopReviewGate`) і незавершені job handles (`running`/`queued`);
   - **Координація з legacy-воркерами (щоб не втратити завершення)**: старий /tmp-воркер, що ще в черзі/виконується, продовжить писати у legacy `state.json`, а нові процеси читатимуть лише XDG → його completion не відобразиться. Мітигація: (а) мігрувати job handles як `running`/`queued`, але позначити їх `legacy:true` + зберегти legacy-шлях; (б) teardown/status для таких job перевіряє legacy-файл (якщо ще не `.migrated`) поки job не термінальний або PID живий; (в) перейменування legacy→`.migrated` виконувати ЛИШЕ коли жоден мігрований job не активний (немає живого PID серед legacy `running`/`queued`) — інакше лишити legacy на місці й домігрувати статуси при наступному `loadState`. Задокументувати обмеження: одночасний up/downgrade під час активних legacy-job не гарантує миттєвої видимості — рекомендувати міграцію при idle.
...
- **Stop-hook на чистому legacy-шляху (проти тихого вимкнення gate)**: з legacy /tmp-станом (`stopReviewGate=true` + running job) і ВІДСУТНІМ новим `state.json` прогнати САМЕ Stop-hook (`stop-review-gate-hook.mjs` entrypoint, не прямий `loadState`) → хук через `initializeState()` мігрує ПЕРЕД `getConfig`, бачить `stopReviewGate=true` (а не default false) і running job у видимості; старий файл → `.migrated`. Аналогічний прогін session-lifecycle entrypoint.
- міграція: старий /tmp-стан з `stopReviewGate=true` + running job → після `initializeState()` новий стан містить той самий config і job handle; старий файл → `.migrated`.
- ідемпотентність: повторний `initializeState()`/`loadState` не дублює і не ремігрує.
- corrupt старий стан → skip + logNote, новий стан = default, без throw.
- безпека міграції (fd-first): старий файл — symlink → `O_NOFOLLOW` кидає `ELOOP` → skip + logNote (немає читання/термінації); підміна шляху на symlink ПІСЛЯ stat-перевірки не читається (перевірка й `readSync` на одному fd — тест мокає, що повторний open за шляхом НЕ викликається); world-writable старий `state.json` (`fstat.mode & 0o022`) → skip; чужий uid (мок `fstat`/`getuid`) → skip. Жоден мігрований PID не потрапляє в новий стан у цих кейсах.
- рекурсивний лок: `loadState`, викликаний зсередини `updateState` (лок уже взято), НЕ ініціює міграцію і не дедлочиться; міграція йде лише окремою `migrateLegacyState()`.
- legacy-координація: живий legacy `running`-job не перейменовує старий файл у `.migrated` (лишається для довидимості); мертвий/термінальний → `.migrated`.
...
Зробити:
1. **Виділити переюзний O_EXCL lock-хелпер** з патерну `job-slots.mjs` (`withFileLock(lockPath, fn)`): `openSync(lockPath,"wx")` створює лок, PID+timestamp+random-token payload пишеться одразу `writeSync`→`closeSync`. retry ~50ms до ~5s.
   - **Не покладатися на не-порожній payload як ознаку валідності** (`openSync("wx")`→`writeSync` НЕ атомарний: порожній lock-файл уже видимий іншим читачам у вікні між create і `writeSync`). Тому: (а) holder перевіряє власність через payload-token, а не лише «файл існує»; (б) reader, що бачить порожній/недописаний lock молодший за grace (mtime < ~2s), трактує його як живий-у-створенні і чекає (retry), НЕ reclaim-ить.
   - **Stale-reclaim серіалізований, TOCTOU-safe (проти взаємного видалення свіжих локів)**: наївний read→check-mtime/pid→`unlinkSync(lockPath)` дозволяє гонку — A і B обидва бачать старий dead-lock, A видаляє й бере новий лок, B потім видаляє вже свіжий лок A, C заходить паралельно (порушення взаємного виключення). Тому reclaim НЕ робить прямий `unlink` за шляхом. Замість цього:
     - reap виконується під ОКРЕМИМ короткоживучим reaper-локом (`<lockPath>.reap`, той самий `openSync("wx")`), щоб рівно один reclaimer діяв одночасно;
     - під reaper-локом: прочитати поточний lock payload → якщо mtime>10s І pid мертвий → зафіксувати `token` прочитаного payload → **rename-based reclaim**: `renameSync(lockPath, <lockPath>.dead.<pid>)` (атомарний «claim» конкретного inode), перечитати перейменований файл і звірити, що `token` збігся (інакше хтось уже підмінив — abort, лишити як є), лише тоді `unlinkSync(.dead)` і взяти новий лок;
     - будь-яке розходження token між pre-check і post-rename → abort reclaim (свіжий лок іншого власника не чіпати).
   - Помістити в `lib/job-slots.mjs` (експорт) або новий `lib/file-lock.mjs`. НЕ власна копія — реюзати перевірену деталь, але з доданим token+reaper-lock протоколом (наявний `job-slots.mjs` reap теж перевести на нього, щоб не лишити стару TOCTOU-гонку).
...
5. **ЄДИНЕ джерело істини статусу job — обов'язкове (два незалежно-CAS'нуті файли розходяться)**. Два окремі файли з незалежним CAS НЕ дають консистентності: сценарій interleaving — cancel записав `jobs/<id>.json=cancelled`, але ще НЕ оновив `state.json`; воркер читає `state.json=running`, і його `completed` — легальний не-термінальний→термінальний перехід за CAS самого `state.json`, тож приймається; per-job CAS відхилив би completion, а `state.json`-CAS прийняв → `cancelled` в одному файлі, `completed` у status-в'юсі. Дві незалежні CAS-перевірки цього не рятують, бо перевіряють різні копії. Тому:
   - **`jobs/<id>.json` — ЄДИНИЙ canonical source статусу job** (не «альтернатива», а вимога). Кожен запис статусу — atomic write-and-rename під **per-job локом** (`jobs/<id>.lock`, той самий хелпер) з **CAS проти щойно-прочитаного під тим самим локом канонічного статусу**: `cancelled`/`completed`/`failed` термінальні; термінальний НЕ перезаписується жодним іншим статусом (ні не-термінальним, ні іншим термінальним — `completed` НЕ затирає `cancelled`). Будь-який писар статусу (cancel, worker-completion, teardown) бере per-job лок, читає канонічний статус, і лише тоді вирішує CAS — тож воркер, що «бачив running» до cancel, під локом перечитає `cancelled` і відхилить `completed`.
   - **Job-статус у `state.json` — ПОХІДНИЙ, не самостійне джерело**: `state.json` не є authority для статусу; при рендері/status-в'юсі статус job береться (або звіряється) з `jobs/<id>.json`. Воркер НЕ пише термінальний статус прямо в `state.json` в обхід per-job CAS. Якщо для сумісності статус усе ж дублюється в `state.json`, він оновлюється ЛИШЕ з канонічного `jobs/<id>.json`-значення (після його CAS), у тій самій логічній операції, і ніколи не «переганяє» канон.
   - Вимога-інваріант: **жоден шлях запису (жоден файл) не може відкотити чи розійтися з канонічним термінальним статусом** — розходження `state.json` vs `jobs/<id>.json` для того самого job у status-в'юсі є багом.
...
- **cancel/complete КОНКУРЕНТНЕ interleaving (не послідовне)**: воркер прочитав статус `running` (зберегти цей snapshot), ПОТІМ cancel робить `cancelled`, ПОТІМ воркер намагається закомітити `completed` зі СТАРИМ snapshot (running) → воркер під per-job локом ПЕРЕЧИТУЄ канон, бачить `cancelled`, CAS відхиляє `completed`. Фінал: `jobs/<id>.json=cancelled` І похідний `state.json`-статус = `cancelled` (НЕ розходяться). Змоделювати саме interleaving: воркер утримує стару snapshot-змінну через хук/затримку між read і write, cancel вклинюється між ними. Перевірити ОБИДВА файли + status-в'юху = `cancelled`.
- **розходження заборонене**: після конкурентної гонки `jobs/<id>.json` і `state.json` для того самого job НЕ показують різний статус (інваріант «немає divergence»).
...
- stale-lock (мертвий pid, mtime>10s) → reclaim через reaper-лок + rename; свіжий лок з живим pid → не reclaim.
- **stale-reclaim TOCTOU**: два конкурентні reclaimer над одним dead-lock → рівно ОДИН reclaim (reaper-лок серіалізує); token-звірка post-rename не збіглася (симуляція підміни свіжим локом між pre-check і rename) → reclaim abort-иться, свіжий лок НЕ видаляється.
- PID/token payload: порожній/недописаний lock-файл молодший за grace (mtime<2s) НЕ reclaim-иться передчасно (трактується як живий-у-створенні); holder звіряє власність по token, а не по «файл існує».
...
Файли: `plugins/antigravity/scripts/lib/process.mjs` (`runCommand` shell — `process.mjs:12`, `binaryAvailable` — `:38`), `plugins/antigravity/scripts/lib/antigravity.mjs` (`spawnAgyTurn` прямий `spawn("agy", …)` — `antigravity.mjs:414`).
...
2. Проблему `.cmd`/`.bat` (git/npm/agy у PATH як `.cmd`) розв'язати **явним резолвом розширення**, НЕ загальним shell-ом: на win32 резолвити повний шлях бінарника через `PATHEXT` (перебір `.exe`/`.cmd`/`.bat`/`.com`) або `where.exe`.
   - **КРИТИЧНО (Windows):** резолв шляху до `.cmd`/`.bat` НЕ робить його прямо виконуваним — `CreateProcess`/`spawnSync` із `shell:false` на `.cmd`/`.bat` падає (`ENOENT`/`STATUS_INVALID_IMAGE_FORMAT`); shim треба запускати через інтерпретатор команд. Тому правило резолву:
     - резолвлений шлях `.exe`/`.com` (або Node-entry) → спавнити напряму з `shell:false`;
     - резолвлений шлях `.cmd`/`.bat` → спавнити `cmd.exe` з `["/d","/s","/c", "<повний-шлях-до-.cmd>", ...args]` і `shell:false`, з **ручним quoting-ом** кожного аргументу за правилами cmd.exe. `cmd.exe /d /s /c` РЕПАРСИТЬ увесь command tail — тому quoting має бути повним, а не «ловити лише `&`»:
       - обгортати кожен аргумент у `"..."` і подвоювати внутрішні `"`;
       - екранувати cmd-метасимволи `& | < > ^ ( )` через `^` (діють навіть у деяких контекстах поза лапками);
       - **`%` і `!` (delayed expansion)**: `%VAR%`/`!VAR!` розкриваються cmd.exe навіть усередині подвійних лапок і НЕ екрануються надійно через `^`. Обробити явно: `%`→неможливо безпечно заекранувати в `/c` контексті без `setlocal` — тому або запускати shim через `cmd.exe /v:off` (вимкнути delayed expansion для `!`) і замінювати `%`→`%%`-безпечний патерн для literal, або (простіше й надійніше) **валідувати вхід і відхиляти аргумент, що містить `%` чи `!`** із чіткою помилкою (git-ref/шляхи їх не потребують);
       - **CR/LF — відхиляти беззастережно**: `\r`/`\n` в аргументі неможливо безпечно передати через `cmd.exe /c` (перенос рядка завершує/додає команду) → валідація відкидає такий аргумент із помилкою, НЕ намагатися заекранувати.
       Quoting/валідацію покрити тестами на ін'єкцію саме для `& | < > ^ ( ) % !` ТА CR/LF, не лише `&`.
   - Аргументи (включно з user-controlled `--base <ref>`) ніколи не інтерпретуються неявним `shell:true`; єдиний дозволений інтерпретатор — явний `cmd.exe /c` із контрольованим quoting/валідацією лише для `.cmd`/`.bat`-shim.
3. **`agy`-спавн ТЕЖ через резолвер (інакше resolver у `process.mjs` його не захищає)**: `spawnAgyTurn` (`antigravity.mjs:414`) наразі робить прямий `spawn("agy", args)` — це (а) на win32 із `shell:false` впаде для `agy.cmd` (`ENOENT`), хоча `binaryAvailable("agy")` через резолвер скаже «available» — розсинхрон доступності й запуску; (б) обходить cmd-quoting. Провести цей `spawn` через той самий `resolveExecutable("agy")` → спавнити `file` з `prefixArgs.concat(args)` (для `.cmd` — `cmd.exe /d /s /c` + quoting/валідація). Резолвер має підтримувати і async `spawn` (не лише `spawnSync`). Availability-проба і фактичний запуск `agy` — через ОДИН механізм резолву.
4. `binaryAvailable` на win32 (`process.mjs:38`) — резолв через той самий PATHEXT/`where.exe`-механізм (напр. знайти `npm.cmd`), без загального shell.
5. Винести резолвер у хелпер (напр. `resolveExecutable(command)` → `{ file, prefixArgs, isBatch }` в `process.mjs`); на POSIX — no-op (повертає command без prefix). `runCommand`/`binaryAvailable`/`spawnAgyTurn` спавнять `file` з `prefixArgs.concat(quotedArgs)`.
...
- win32 (мок platform): резолв `git`/`npm` через PATHEXT/`where.exe` знаходить `.cmd`; `.cmd`-shim спавниться через `cmd.exe /d /s /c <шлях>` (НЕ напряму), `shell:false` — перевірити argv[0]==="cmd.exe" і що резолвлений `.cmd` передано як аргумент, а не як `file` прямого спавну.
- win32 `.exe`-резолв → прямий спавн без `cmd.exe`-префікса.
- win32 injection (ПОВНИЙ набір метасимволів, не лише `&`): `--base` зі значеннями, що містять кожен із `& | < > ^ ( )` через `.cmd`-шлях → cmd-quoting обгортає/екранує, друга команда НЕ виконується (перевірити quoted-argv на коректне екранування КОЖНОГО).
- win32 `%`/`!` у аргументі → валідація відхиляє з чіткою помилкою (або, якщо обрано `/v:off`+`%%`-шлях — перевірити, що `%VAR%`/`!VAR!` не розкрилися); CR/LF у аргументі → відхилення з помилкою (НЕ спроба заекранувати).
- **`agy`-спавн через резолвер**: win32 (мок) `spawnAgyTurn` резолвить `agy`→`agy.cmd` і спавнить через `cmd.exe /d /s /c <шлях-agy.cmd>`, НЕ прямий `spawn("agy")`; перевірити, що `spawn` викликано з `file==="cmd.exe"`. win32 `.exe`-варіант agy → прямий спавн. POSIX `agy` → прямий спавн без префікса (регрес-контроль).
- **availability/spawn консистентність**: якщо `binaryAvailable("agy")` резолвить через хелпер, то `spawnAgyTurn` використовує ТОЙ САМИЙ резолв (немає кейсу «available, але spawn ENOENT»).
- `binaryAvailable` win32 знаходить `npm.cmd`.

Verify: `npm test` зелений; grep `shell:` в `process.mjs` — лише `false` (або відсутнє); grep `process.env.SHELL` — прибрано; grep `spawn("agy"` / `spawn('agy'` в `antigravity.mjs` — прямого бару нема (замінено на резолвлений `file`).
...
### D3. B8 foreground timeout — каскад НЕ інвертований (зовнішній Bash > внутрішній wrapper)
...
Проблема (та сама інверсія, що B2 виправляє для Stop-hook): зовнішній Bash `timeout: 600000` (600s) МЕНШИЙ за внутрішній runner-дефолт `DEFAULT_TURN_TIMEOUT_MS = 900000` (`antigravity.mjs:41`). На 10-й хвилині Bash уб'є wrapper РАНІШЕ, ніж той сформує контрольовану timeout-помилку → та сама гонка. З A1-repair-turn сумарний час wrapper ще більший (initial turn + до одного repair turn), тож 900s-per-turn тим паче не влазить у 600s.

Зробити: зовнішній Bash timeout МАЄ строго перевищувати ПОВНИЙ внутрішній бюджет wrapper (initial turn + можливий A1 repair turn + overhead), щоб wrapper self-timeout-нувся першим. Оскільки Bash `timeout` у Claude Code обмежений стелею 600000ms, зменшити ВНУТРІШНІЙ бюджет під цю стелю, а не навпаки:
1. Foreground-блоки виставляють Bash `timeout: 600000`.
2. Той самий виклик експортує `ANTIGRAVITY_COMPANION_TURN_TIMEOUT_MS` (env-читалка з B2) значенням, за якого НАЙГІРШИЙ час життя wrapper (initial + один repair turn + overhead) лишається строго `< 600000` — напр. per-turn `≤ 270000` (2×270s ≈ 540s < 600s), або (надійніше) wrapper дотримується ОДНОГО загального deadline на всю послідовність turn-ів (не per-turn), виставленого нижче Bash-стелі. Джерело правди порядку — «зовнішній Bash > повний внутрішній бюджет».
3. Узгодити з B2 (Stop-hook каскад 780<840<900) і A1 (repair-turn входить у внутрішній бюджет): у foreground середовищі 900s-per-turn НЕ використовувати — переоприділяти env-ом.

Verify: grep `600000` у відповідних foreground-блоках; у тих самих блоках присутній `ANTIGRAVITY_COMPANION_TURN_TIMEOUT_MS` зі значенням, що дає повний внутрішній бюджет `< 600000` (не інвертовано); документація явно каже, що зовнішній Bash > внутрішній wrapper.
