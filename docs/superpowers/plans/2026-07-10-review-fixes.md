...
4. **Одноразова міграція (обов'язкова)**: при першому `loadState` з новим шляхом, якщо новий `state.json` відсутній, а старий /tmp-шлях (`os.tmpdir()/antigravity-companion/<slug>-<hash>/state.json`) читабельний:
   - **Безпекова передумова (обов'язкова, ДО читання вмісту — TOCTOU-safe через `lstat`)**: старий каталог І старий `state.json` мають (а) належати поточному користувачу (`lstatSync(...).uid === process.getuid()`), (б) НЕ бути symlink-ами (`!lstat.isSymbolicLink()` на кожному компоненті — і каталог, і файл), (в) мати безпечні права (не world-writable: `(mode & 0o022) === 0`). Будь-яка умова не виконана → **пропустити міграцію** з `logNote` (не читати, не термінувати PID), бо /tmp world-writable: атакер міг підкласти `state.json` із чужими job-PID (напр. системних сервісів), і teardown викликав би `terminateProcessTree` на них (LPE/arbitrary-kill). На платформах без `getuid` (win32) — пропускати перевірку uid, але symlink+permission-перевірки лишаються.
   - **Уникнути рекурсивного локу (O_EXCL нереентрантний)**: `loadState` НЕ виконує міграцію через `updateState`/публічний `saveState` (вони беруть `state.lock`; якщо `loadState` викликаний уже під локом з `updateState`→read, повторний захват задедлочить). Міграція виконується в окремій явній точці входу (напр. `migrateLegacyState()`), яку викликає ЛИШЕ верхньорівневий не-локнутий шлях (перший `ensureStateDir`/старт процесу), і пише через `saveStateUnlocked` під ОДНИМ локом, взятим саме цією точкою. `loadState` лишається чистою читалкою без побічної міграції.
...
- безпека міграції: старий каталог/файл — symlink → skip + logNote (немає читання/термінації); world-writable старий `state.json` (mode & 0o022) → skip; чужий uid (мок `getuid`/`lstat`) → skip. Жоден мігрований PID не потрапляє в новий стан у цих кейсах.
...
5. **Per-job файли `jobs/<id>.json`** (`writeJobFile` — `state.mjs:166-171`): кожен запис — atomic write-and-rename під **per-job локом** (`jobs/<id>.lock`, той самий хелпер), і переходи статусу — **CAS/монотонні**: `cancelled`/`completed`/`failed` термінальні; термінальний не перезаписується не-термінальним; `completed` не затирає `cancelled`.
   - **Той самий CAS-гард ОБОВ'ЯЗКОВО застосовується і до job-записів у `state.json`** (`upsertJob`/будь-який write job-статусу в canonical `state.json`), не лише до `jobs/<id>.json`. Інакше гонка: user скасовує → `cancelled` збережено в `jobs/<id>.json` через CAS, але воркер завершується і його `upsertJob` перезаписує `state.json` на `completed`; status-в'юхи читають `state.json` → показують `completed`, скасування «зникає». Отже `updateState`/`saveStateUnlocked` при мутації job-статусу мають читати поточний статус того job у СВІЖОМУ стані (під тим самим `state.lock`) і **відхиляти не-термінальний→над-термінальним та термінальний→інший-термінальний перехід** (зокрема `completed` НЕ затирає `cancelled`). Джерело істини статусу — узгоджене між `state.json` і `jobs/<id>.json`.
   - (Альтернатива, якщо простіше: `jobs/<id>.json` — єдиний canonical source для статусів із CAS, а job-статус у `state.json` — похідний, що читається/рендериться з `jobs/<id>.json`, а не пишеться воркером напряму; вибір гілки на етапі імплементації — вимога: жоден шлях запису не може відкотити термінальний статус.)
...
- cancel/complete гонка: job `cancelled`, потім воркерів `upsertJob('completed')` у `state.json` → фінальний статус у `state.json` лишається `cancelled` (термінальний не відкочується); status-в'юха показує `cancelled`, не `completed`.
...
2. Проблему `.cmd`/`.bat` (git/npm/agy у PATH як `.cmd`) розв'язати **явним резолвом розширення**, НЕ загальним shell-ом: на win32 резолвити повний шлях бінарника через `PATHEXT` (перебір `.exe`/`.cmd`/`.bat`/`.com`) або `where.exe`.
   - **КРИТИЧНО (Windows):** резолв шляху до `.cmd`/`.bat` НЕ робить його прямо виконуваним — `CreateProcess`/`spawnSync` із `shell:false` на `.cmd`/`.bat` падає (`ENOENT`/`STATUS_INVALID_IMAGE_FORMAT`); shim треба запускати через інтерпретатор команд. Тому правило резолву:
     ...
     - резолвлений шлях `.cmd`/`.bat` → спавнити `cmd.exe` з `["/d","/s","/c", "<повний-шлях-до-.cmd>", ...args]` і `shell:false`, з **ручним quoting-ом** кожного аргументу за правилами cmd.exe (обгортання в `"..."`, екранування `"` та cmd-метасимволів `& | < > ^ ( ) %`), щоб user-controlled `--base <ref>` не міг вирватись у виконання другої команди. Quoting-функцію покрити тестами на ін'єкцію.
   - Аргументи (включно з user-controlled `--base <ref>`) ніколи не інтерпретуються неявним `shell:true`; єдиний дозволений інтерпретатор — явний `cmd.exe /c` із контрольованим quoting лише для `.cmd`/`.bat`-shim.
3. `binaryAvailable` на win32 (`process.mjs:38`) — резолв через той самий PATHEXT/`where.exe`-механізм (напр. знайти `npm.cmd`), без загального shell.
4. Винести резолвер у хелпер (напр. `resolveExecutable(command)` → `{ file, prefixArgs, isBatch }` в `process.mjs`); на POSIX — no-op (повертає command без prefix). `runCommand`/`binaryAvailable` спавнять `file` з `prefixArgs.concat(quotedArgs)`.
...
- win32 injection: `--base "a & calc"` через `.cmd`-шлях → cmd-quoting обгортає/екранує метасимвол, друга команда НЕ виконується (перевірити quoted-argv, немає невекранованого `&`).
